...
- Plugin (Claude Code, Claude desktop app, Cowork, Gemini CLI, Codex, GitHub Copilot CLI): `https://aidesignblueprint.com/en/for-agents/plugin`
...
3. **iter9 cert**: **downgraded to emerging**. Cert caught that `@dataclass`, `Literal`, `json`, `datetime`, `timezone` were USED in the payload but never IMPORTED, so the module would NameError on import. Real on-disk code had the imports; submitted payload had dropped them. Cert was correct to refuse.
