...
1. Install the Cursor Rule Pack: `curl -L -o /tmp/cursor-rule-pack.zip https://aidesignblueprint.com/agent-assets/cursor-rule-pack.zip && unzip -o /tmp/cursor-rule-pack.zip -d .`. It installs `.cursor/rules/blueprint-doctrine.mdc` and `.cursor/mcp.json` at project root.
